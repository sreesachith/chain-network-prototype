...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating rr Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-rr.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating ec Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-ec.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "This step done"
    while :
    do
      if [ ! -f "organizations/fabric-ca/rr/tls-cert.pem" ]; then
        infoln "Struck in IF"
        sleep 1
      else
        break
  ...
  infoln "Generating CCP files for rr and ec"
...
function networkUp() {
  ...
  infoln "Done generating orgs"

  #orgiginal#COMPOSE_FILES="-f compose/${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  COMPOSE_FILES="-f compose/${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"

  if [ "${DATABASE}" == "couchdb" ]; then
    #COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${COMPOSE_FILE_COUCH}"
  fi
...
# call the script to create the channel, join the peers of rr and ec,
...
function networkDown() {
  ...
  COMPOSE_COUCH_FILES="-f compose/${COMPOSE_FILE_COUCH} -f compose/${COMPOSE_FILE_COUCH}"
  COMPOSE_CA_FILES="-f compose/${COMPOSE_FILE_CA} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}"
  COMPOSE_FILES="${COMPOSE_BASE_FILES} ${COMPOSE_COUCH_FILES} ${COMPOSE_CA_FILES}"

  # stop org3 containers also in addition to rr and ec, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm compose_orderer.isfcr.com compose_peer0.rr.isfcr.com compose_peer1.rr.isfcr.com compose_peer2.rr.isfcr.com compose_peer0.ec.isfcr.com compose_peer1.ec.isfcr.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/rr/msp organizations/fabric-ca/rr/tls-cert.pem organizations/fabric-ca/rr/ca-cert.pem organizations/fabric-ca/rr/IssuerPublicKey organizations/fabric-ca/rr/IssuerRevocationPublicKey organizations/fabric-ca/rr/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ec/msp organizations/fabric-ca/ec/tls-cert.pem organizations/fabric-ca/ec/ca-cert.pem organizations/fabric-ca/ec/IssuerPublicKey organizations/fabric-ca/ec/IssuerRevocationPublicKey organizations/fabric-ca/ec/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer/msp organizations/fabric-ca/orderer/tls-cert.pem organizations/fabric-ca/orderer/ca-cert.pem organizations/fabric-ca/orderer/IssuerPublicKey organizations/fabric-ca/orderer/IssuerRevocationPublicKey organizations/fabric-ca/orderer/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=test-net.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=couch.yaml
